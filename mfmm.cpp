...
ModifiedFastMarchingMethod::ModifiedFastMarchingMethod(FIELD<float>* f_, FLAGS* flags_, IMAGE<float>* image_,
                                                       FIELD<float>* gx, FIELD<float>* gy, FIELD<float>* d, int br,
                                                       int dst_wt, int lev_wt, int N_)
    : FastMarchingMethod(f_, flags_, N_), image(image_), grad_x(gx), grad_y(gy), dist(d), B_radius(br), dst_weighting(dst_wt), lev_weighting(lev_wt) {
}
...
void ModifiedFastMarchingMethod::add_to_narrowband(int i, int j, int active_i, int active_j) {
    ...
    for (ii = -N; ii <= N; ii++) //look at the known pixels in a window around current-point
        for (jj = -N; jj <= N; jj++) {
            if (!flags->alive(i + ii, j + jj))
                continue; //work on known pixels only...
            if (ii == 0 || jj == 0)
                continue; //skip current point, we inpaint it
            float dirx = -ii;
            float diry = -jj; //project direction (i,j)->current-point on image gradient
            ...
            r = fabs(r);
            if (dst_weighting)
                r /= dd * dd; //do distance weighting (optional)
            if (lev_weighting)
                r /= (1 + (dst - dst0) * (dst - dst0)); //do level-weighting (optional)
    ...
    if (cnt == 0 || cntx == 0 || cnty == 0) //occurs sometimes when B_radius very small (e.g. 1)
    {
        ...
        gx_r /= cntx;
        gy_r /= cnty; //normalize avg-gradient
        gx_g /= cntx;
        gy_g /= cnty;
        gx_b /= cntx;
        gy_b /= cnty;
...
int ModifiedFastMarchingMethod::diffuse() {
    ...
    if (f->value(min_i, min_j) >= maxf)
        return 1; //stop evolution if we reached the user-prescribed threshold.
