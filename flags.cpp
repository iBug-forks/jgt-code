...
int FLAGS::connected(int min_i, int min_j) const {
    if (faraway(min_i - 1, min_j))
        return 1;
    if (faraway(min_i, min_j - 1))
        return 1;
    if (faraway(min_i + 1, min_j))
        return 1;
    if (faraway(min_i, min_j + 1))
...
FIELD<int>* FLAGS::tagConnect(int* c_) const //create field coding points by cluster-number
{
    ...
    for (int j = 0; j < f->dimY(); j++) //for every pixel compute its connectivity
        for (int i = 0; i < f->dimX(); i++)
            if (!alive(i, j) && f->value(i, j) == CONN_UNKNOWN) { //if pixel belongs to some cluster, but is not yet labelled,
                flood_fill(*f, i, j, c);                          //do a flood-fill from it with current label
                c++;                                              //advance the label
            }

    if (c_)
        *c_ = c; //return #clusters if desired

    return f;
}

int FLAGS::flood_fill(FIELD<int>& f, int i, int j, int val) const { //Fills all pixels connected (i.e. with same value)
    ...
    while (s.Count()) //For all pixels to fill from:
    {
        ...
        if (alive(c.i, c.j))
            continue; //If pixel is background continue
        f.value(c.i, c.j) = val;
        k++; //Fill current pixel with 'val'

        if (c.i > 0 && f.value(c.i - 1, c.j) != val)
            s.Push(Coord(c.i - 1, c.j));
        if (c.i < nx && f.value(c.i + 1, c.j) != val)
            s.Push(Coord(c.i + 1, c.j));
        if (c.j < ny && f.value(c.i, c.j + 1) != val)
            s.Push(Coord(c.i, c.j + 1));
        if (c.j > 0 && f.value(c.i, c.j - 1) != val)
            s.Push(Coord(c.i, c.j - 1));
    }
