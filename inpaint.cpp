...
float w[5][5] = {{S8, 0, SG, 0, S8}, {0, 0, S4, 0, 0}, {SG, S4, 1 + 4 * G + 12.5 * S, S4, SG}, {0, 0, S4, 0, 0}, {S8, 0, SG, 0, S8}};
...
int main(int argc, char* argv[]) {
    ...
    } else if (argc < 2) //One arg given, namely the input-image
    {
        ...
        strcpy(inp, argv[0]);
        argc--;
        argv++; //arg1: name of scalar field
        strcpy(img, argv[0]);
        argc--;
        argv++;
    ...
    if (!inp[0])
        sprintf(inp, "scratch_%s", img); //if no scratch image given, infer its name from original image

    f = FIELD<float>::read(inp); //read scratch image
    if (!f) {
        cout << "Can not open file: " << inp << endl;
        return 1;
    }
    rgb_image = IMAGE<float>::read(img);
    if (!rgb_image) {
        cout << "Can not open file: " << img << endl;
        return 1;
    }
...
FIELD<float>* compute_distance(FIELD<float>* fi, float k, float maxd) {
    ...
    for (int i = 0; i < f->dimX(); i++)
        for (int j = 0; j < f->dimY(); j++) {
            if (fcopy->alive(i, j))
                f->value(i, j) = -fout->value(i, j);
            if (flagsout->faraway(i, j))
                f->value(i, j) = 0;
        }
...
void compute_gradient(FIELD<float>* f, FIELD<float>*& gx, FIELD<float>*& gy) { //compute gradient of 'f' in 'gx','gy'
    gx = new FIELD<float>(f->dimX(), f->dimY());
    *gx = 0;
    gy = new FIELD<float>(f->dimX(), f->dimY());
    *gy = 0;
...
void draw(IMAGE<float>& f) {
    ...
    f.r.minmax(mr, Mr, avg);
    f.g.minmax(mg, Mg, avg);
    f.b.minmax(mb, Mb, avg);
    const float *dr = f.r.data(), *dg = f.g.data(), *db = f.b.data();
    int j = 0;
    for (int i = 0; i < f.dimY(); i++)
        for (const float *sr = dr + f.dimX() * i,
                         *sg = dg + f.dimX() * i,
                         *sb = db + f.dimX() * i, *e = sr + f.dimX();
             sr < e; sr++, sg++, sb++) {
            float r = ((*sr) - mr) / (Mr - mr),
                  g = ((*sg) - mg) / (Mg - mg),
                  b = ((*sb) - mb) / (Mb - mb);
