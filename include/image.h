...
template <class T>
IMAGE<T>* IMAGE<T>::read(char* fname) {
...
template <class T>
inline void IMAGE<T>::setValue(int i, int j, T v) { r.value(i, j) = g.value(i, j) = b.value(i, j) = v; }

template <class T>
IMAGE<T>* IMAGE<T>::readBMP(char* fname) {
    fstream inf;
...
template <class T>
unsigned long IMAGE<T>::getLong(fstream& inf) {
    unsigned long ip;
    char ic;
    unsigned char uc;
    inf.get(ic);
...
template <class T>
unsigned short IMAGE<T>::getShort(fstream& inf) {
    char ic;
    unsigned short ip;
    inf.get(ic);
    ip = ic;
