...
class VFIELD {
   public:
    ...
    void size(int i, int j) {
        v0.size(i, j);
        v1.size(i, j);
    }
    void size(const FIELD<T>& f) {
        v0.size(f);
        v1.size(f);
    }
...
template <class T>
inline FIELD<T>::FIELD(int nx_, int ny_) : nx(nx_), ny(ny_), v((nx_ * ny_) ? new T[nx_ * ny_] : 0) {}

template <class T>
inline FIELD<T>::FIELD(const FIELD<T>& f) : nx(f.nx), ny(f.ny), v((f.nx * f.ny) ? new T[f.nx * f.ny] : 0) {
    if (nx * ny)
        memcpy(v, f.v, nx * ny * sizeof(T));
}

template <class T>
inline T& FIELD<T>::value(int i, int j) {
...
template <class T>
inline const T FIELD<T>::value(float i, float j) const {
...
template <class T>
inline const T& FIELD<T>::value(int i, int j) const {
    i = (i < 0) ? i : (i >= nx) ? 2 * nx - i - 1 : i;
    j = (j < 0) ? j : (j >= ny) ? 2 * ny - j - 1 : j;
    return *(v + j * nx + i);
}
...
template <class T>
inline T FIELD<T>::gradnorm(int i, int j) const {
...
template <class T>
inline FIELD<T>::~FIELD() { delete[] v; }

template <class T>
inline void FIELD<T>::size(const FIELD& f) {
    delete[] v;
    nx = f.nx;
    ny = f.ny;
...
template <class T>
FIELD<T>& FIELD<T>::operator=(FIELD& f) {
    if (nx != f.nx || ny != f.ny)
        size(f);
    if (nx * ny)
        memcpy(v, f.v, nx * ny * sizeof(T));
...
template <class T>
FIELD<T>& FIELD<T>::operator+=(const FIELD& f) {
...
template <class T>
FIELD<T>& FIELD<T>::operator=(T val) {
...
template <class T>
void FIELD<T>::gradnorm(FIELD& f) const {
    f.size(*this);
...
template <class T>
void FIELD<T>::minmax(T& m, T& M, T& a) const {
...
template <class T>
FIELD<T>* FIELD<T>::read(char* fname) {
    switch (fileType(fname)) {
        case VTK:
            return readVTK(fname);
        case BMP:
...
template <class T>
unsigned long FIELD<T>::getLong(fstream& inf) {
    unsigned long ip;
    char ic;
    unsigned char uc;
    inf.get(ic);
...
template <class T>
unsigned short FIELD<T>::getShort(fstream& inf) {
    char ic;
    unsigned short ip;
    inf.get(ic);
    ip = ic;
...
template <class T>
FIELD<T>* FIELD<T>::readBMP(char* fname) {
    fstream inf;
    ...
    for (unsigned int row = 0; row < numRows; row++) //for every row
    {
        ...
        for (unsigned int k = 0; k < numPadBytes; k++)
            inf >> ch; //skip pad bytes at end of row
    }
    inf.close();
    return f;
}
...
template <class T>
FIELD<T>* FIELD<T>::readVTK(char* fname) //read VTK scalar file into this
{
    FILE* fp = fopen(fname, "r");
    if (!fp)
        return 0;
...
template <class T>
FIELD<T>* FIELD<T>::readPGM(char* fname) //read VTK scalar file into this
{
    FILE* fp = fopen(fname, "r");
    if (!fp)
        return 0;
    ...
    {                                                            //be careful: buf is a char, we first need
        if (bb == SIZE) {
            fread(buf, SIZE, 1, fp);
            bb = 0;
        }                              //to convert the read bytes to unsigned char and then assign
...
template <class T>
FIELD<T>* FIELD<T>::readASCII(char* fname) //read plain ASCII file into this
{
    FILE* fp = fopen(fname, "r");
    if (!fp)
        return 0;
...
template <class T>
void FIELD<T>::writeGrid(char* fname) {
    FILE* fp = fopen(fname, "w");
    if (!fp)
        return;
...
template <class T>
void FIELD<T>::writePGM(char* fname) const {
    FILE* fp = fopen(fname, "w");
    if (!fp)
        return;
...
template <class T>
void FIELD<T>::writePPM(char* fname) const {
    FILE* fp = fopen(fname, "w");
    if (!fp)
        return;
...
template <class T>
inline void VFIELD<T>::setValue(int i, int j, T* val) {
    v0.value(i, j) = val[0];
    v1.value(i, j) = val[1];
}

template <class T>
VFIELD<T>* VFIELD<T>::read(char* fname) {
    FILE* fp = fopen(fname, "r");
    if (!fp)
        return 0;
...
template <class T>
void VFIELD<T>::write(const char* fname, FLAGS& f) const {
    FILE* fp = fopen(fname, "w");
    if (!fp)
        return;
...
template <class T>
void VFIELD<T>::write(const char* fname) const {
    FILE* fp = fopen(fname, "w");
    if (!fp)
        return;
