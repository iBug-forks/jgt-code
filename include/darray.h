...
template <class T, int GROW_SIZE = 20>
class DARRAY //DARRAY is parametrized by a GROW_SIZE factor
...
inline DARRAY<T, GROW_SIZE>::DARRAY(int N)
    : a(new T[N]), n(0), sz(N) {}

template <class T, int GROW_SIZE>
DARRAY<T, GROW_SIZE>::DARRAY(const DARRAY& arr)
    : a(new T[arr.sz]), n(arr.n), sz(arr.sz) {
    for (T *i = a, *end = a + sz, *av = arr.a; i < end; i++, av++)
        *i = *av;
}

template <class T, int GROW_SIZE>
inline DARRAY<T, GROW_SIZE>::~DARRAY() { delete[] a; }
...
DARRAY<T, GROW_SIZE>& DARRAY<T, GROW_SIZE>::operator=(const T& t) {
    for (T *i = a, *end = a + n; i < end; i++)
        *i = t;
    return *this;
}

template <class T, int GROW_SIZE>
inline void DARRAY<T, GROW_SIZE>::Init(int N) {
    delete[] a;
    a = new T[sz = N];
    n = 0;
}
...
template <class T, int GROW_SIZE>
inline int DARRAY<T, GROW_SIZE>::Count() const { return n; }

template <class T, int GROW_SIZE>
inline void DARRAY<T, GROW_SIZE>::SetCount(int c) { n = c; }

template <class T, int GROW_SIZE>
inline void DARRAY<T, GROW_SIZE>::InitC(int N) {
    Init(N);
    SetCount(N);
}

template <class T, int GROW_SIZE>
inline T& DARRAY<T, GROW_SIZE>::operator[](int i) const { return a[i]; }

template <class T, int GROW_SIZE>
inline T& DARRAY<T, GROW_SIZE>::operator[](int i) { return a[i]; }
...
template <class T, int GROW_SIZE>
inline void DARRAY<T, GROW_SIZE>::Insert(int i, T x) {
    if (i == -1)
        Add(x);
    else
        INSERT(i, x);
...
template <class T, int GROW_SIZE>
void DARRAY<T, GROW_SIZE>::Delete(int i) {
    if (n) {
        n--;
...
template <class T, int GROW_SIZE>
inline int DARRAY<T, GROW_SIZE>::Remove(T x) {
    int i, ret;
    if (n == 0)
        return -1; //empty array, remove fails

    for (i = 0; i < n; i++)
        if (a[i] == x)
            break; //search for x
    if (i == n)
        return -1; //x not found, remove fails
    ret = i;       //x found at position i
    n--;           //one less element
    for (; i < n; i++)
        a[i] = a[i + 1]; //SHL array from i to end
    return ret;          //return pos of removed element
}

template <class T, int GROW_SIZE>
inline int DARRAY<T, GROW_SIZE>::RemoveAll(T x) {
    int c;
    for (c = 0; Remove(x) != -1;)
        c++;
    return c;
...
template <class T, int GROW_SIZE>
inline void DARRAY<T, GROW_SIZE>::Add(T x) {
...
template <class T, int GROW_SIZE>
inline void DARRAY<T, GROW_SIZE>::operator+=(T x) {
    a[n++] = x; //REMARK: this method assumes there's enough space in array
}

template <class T, int GROW_SIZE>
inline void DARRAY<T, GROW_SIZE>::AddOrd(T x) {
    if (!n) {
        Add(x);
        return;
    } //empty array: just intsert
    ...
    do {
        ...
        register T y = a[m];
        if (x == y)
            return; //found x at position 'm', just return
        if (x < y)
            r = --m;
        else
            l = ++m;
...
template <class T, int GROW_SIZE>
inline int DARRAY<T, GROW_SIZE>::FindOrd(T x) const {
    if (n) //empty array: no search
    {
        ...
        do {
            ...
            register T y = a[m];
            if (x == y)
                return m; //found x at position 'm'
            if (x < y)
                r = --m;
            else
                l = ++m;
...
template <class T, int GROW_SIZE>
inline int DARRAY<T, GROW_SIZE>::Append(T x) {
    for (int i = 0; i < n; i++)
        if (a[i] == x)
            return 0; //element already in there
    Add(x);
    return 1; //element was added
}

template <class T, int GROW_SIZE>
inline void DARRAY<T, GROW_SIZE>::add_and_grow(T x)
...
template <class T, int GROW_SIZE>
int DARRAY<T, GROW_SIZE>::Contains(T x) const {
    for (int i = 0; i < n; i++)
        if (a[i] == x)
            return i;
    return -1;
...
template <class T, int GROW_SIZE>
inline void DARRAY<T, GROW_SIZE>::Flush() { n = 0; }
