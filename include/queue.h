...
template <class T>
inline QUEUE<T>::~QUEUE() { delete[] queue; }
...
template <class T>
inline int QUEUE<T>::Count() { return count; }

template <class T>
inline int QUEUE<T>::Contains(T ptr) {
    int i = g;
    while (i != p) {
        if (queue[i++] == ptr)
            return i;
...
template <class T>
inline T QUEUE<T>::operator[](int i) { return queue[(g + i) % maxsize]; }

template <class T>
inline void QUEUE<T>::Flush() { p = g = count = 0; }
